...
const EmailForm = () => {
  ...
  const [submitState, setSubmitState] = useState<{ type: 'idle' | 'success' | 'error' | 'sending'; message: string }>({
    type: 'idle',
    message: '',
  })
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
   ...
   // alert(`Service ID: ${serviceId}, Template ID: ${templateId}, Public Key: ${publicKey}`); // Debugging line
    ...
    try {
      ...
      const emailJsError = error && typeof error === 'object' && 'text' in error ? String((error as { text?: string }).text) : ''
